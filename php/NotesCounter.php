...
try{
    ...
    function NotesCol($gametime,$notesdata,$id, $notesSt, $notesId){
        ...
        if(isset($notesdata)){
         $noteslength = count($notesdata);//要素数取得
        }else $noteslength=0;
        
        if($notesSt != "miss"){
            //タイミングの計算
            $count = 0;
            foreach($notesdata as $battleNotes){
                if($battleNotes["judge"]==="ALWAY"){ //未処理のノーツから検査
                    if((int)$battleNotes["lernID"] === $id){
                        ...
                        $comp=false;
                        for($i=0; $i<4; $i++){
                            if(abs($timejudge) <= $timingZone[$borderName[$i]]["judget"]){    
                                $notesdata[$count]["judge"] = $borderName[$i];
                                $score +=$timingZone[$borderName[$i]]["upScore"];
                                $taregtNotes=$notesdata[$count];
                                $comp=true;
                                break;
                            }
                        }
            ...
            $notesdata[$notesId]["judge"] = "MISS";
            $taregtNotes=$notesdata[$notesId];
        }
        
        //コンボ数の更新
        $comboDt=CntComb($notesdata,$hanteicount);
        ...
        $_SESSION["notesdata"]=$notesdata;//ノーツデータ更新
