...
abstract class BaseBattler{
    ...
    public function Attack($target){
        ...
        $targetId = $this->targetSet($target, $this->partyIndex);
        $BaseDamage=(int)($this->Characterparam["pow"]*$this->BattleTypeCheck($target[$targetId]->getParameta()["type"]));

        $atkDamageMes = $target[$targetId]->Damaged($BaseDamage);

        $actmes = $this->characterName."の攻撃<br>";
        $actmes .= $atkDamageMes;//$target[$targetId]->characterName."に<strong class=damage>".$atkDamageMes."</strong>ダメージ与えた<br>";

        return [$target, $actmes];
    }
