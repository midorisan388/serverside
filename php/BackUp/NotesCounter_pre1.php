            ...
            foreach($battleNotesdatas as $battleNotes){
                if($battleNotes["judge"]==="ALWAY"){ //未処理のノーツから検査
                    if((int)$battleNotes["lernID"] === $id){
                        ...
                        /*if( $timejudge > $timingZone["BAD"]["judget"]){ //許容時間超過
                        ...
                        for($i=0; $i<5; $i++){
                            if($isIllegal===false){
                                ...
                                $battleNotesdatas[$count]["judge"] = $isIllegal[0];
                                if($isIllegal[0] != "MISS") $score +=$timingZone[$isIllegal[0]]["upScore"];
